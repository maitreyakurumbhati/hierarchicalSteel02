...
class HierarchicalInferenceSampling:
    def __init__(self, coupon_strains, coupon_stresses, start_point,  n_samples, burn_in, tuning_interval):
        ...
        self.n_coupons = len(self.coupon_strains)
    ...
    def perform_mcmc_within_gibbs(self):
        ...
        for sample_counter in range(1, self.n_samples + 1):
            ...
            #
            mh_outputs = current_pool.map(mh_sampler.perform_mh, coupons_mh_input)
            ...
            cov_sample = hierarchical_standard_distributions.\
                get_inverse_wishart_sample(np.shape(S_term)[0] + self.n_coupons, S_term + scale_matrix)
            ...
            mu_theta_sample = hierarchical_standard_distributions.\
                get_multivariate_normal_sample(np.asarray(temp_samp_mean), cov_sample/self.n_coupons)
            ...
            for coupon_counter in range(len(coupon_dict)):
                ...
                coupon_noise_samples.append(hierarchical_standard_distributions.
                               get_scaled_inverse_chi_squared_sample(posterior_dof_sigma, posterior_scale_sigma))

            sample.append(coupon_noise_samples)

            trace.append(sample)

        trace_to_return = trace[self.burn_in:]
